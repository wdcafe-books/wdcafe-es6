...
만나서 반가워요!`;
    ...
    // let num = prompt('주민번호 뒷자리 첫글자를 입력하세요.', '1~4사이 정수입력');
    // let gender = `성별 : ${(num == 1 ? '남성' : '여성')}`;
    ...
    /* 
      Tagged Template Literals는 JavaScript의 특별한 기능 중 하나로, 템플릿 리터럴 (Template Literals)과 함께 사용됩니다. 템플릿 리터럴은 역따옴표 (backtick) `를 사용하여 
      문자열을 생성하는 문법입니다. Tagged Template Literals는 이러한 템플릿 리터럴 앞에 함수를 호출하는 것입니다. 이 함수를 "태그 함수"라고 부릅니다. 
      이 함수는 템플릿 리터럴을 해석하고 처리하는 역할을 합니다. 
    ...
    */
    ...
    function myTagFunction2(str, name1) {
      console.log(str[0] + name1); 
    }
